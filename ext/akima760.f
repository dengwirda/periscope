...
C
C Rectangular-grid bivariate interpolation
C (a master subroutine of the RGBI3P/RGSF3P subroutine package)
C
C Hiroshi Akima
C U.S. Department of Commerce, NTIA/ITS
C Version of 1995/08
C
C This subroutine performs interpolation of a bivariate function,
C z(x,y), on a rectangular grid in the x-y plane.  It is based on
C the revised Akima method.
C
C In this subroutine, the interpolating function is a piecewise
C function composed of a set of bicubic (bivariate third-degree)
C polynomials, each applicable to a rectangle of the input grid
C in the x-y plane.  Each polynomial is determined locally.
C
C This subroutine has the accuracy of a bicubic polynomial, i.e.,
C it interpolates accurately when all data points lie on a
C surface of a bicubic polynomial.
C
C The grid lines can be unevenly spaced.
C
C The input arguments are
C   MD  = mode of computation
C       = 1 for new XD, YD, or ZD data (default)
C       = 2 for old XD, YD, and ZD data,
C   NXD = number of the input-grid data points in the x
C         coordinate (must be 2 or greater),
C   NYD = number of the input-grid data points in the y
C         coordinate (must be 2 or greater),
C   XD  = array of dimension NXD containing the x coordinates
C         of the input-grid data points (must be in a
C         monotonic increasing order),
C   YD  = array of dimension NYD containing the y coordinates
C         of the input-grid data points (must be in a
C         monotonic increasing order),
C   ZD  = two-dimensional array of dimension NXD*NYD
C         containing the z(x,y) values at the input-grid data
C         points,
C   NIP = number of the output points at which interpolation
C         of the z value is desired (must be 1 or greater),
...
C   ZI  = array of dimension NIP where the interpolated z
C         values at the output points are to be stored,
C   IER = error flag
C       = 0 for no errors
C       = 1 for NXD = 1 or less
C       = 2 for NYD = 1 or less
C       = 3 for identical XD values or
C               XD values out of sequence
C       = 4 for identical YD values or
C               YD values out of sequence
C       = 5 for NIP = 0 or less.
C
C The other argument is
C   WK  = three dimensional array of dimension 3*NXD*NYD used
C         internally as a work area.
C
C The very fisrt call to this subroutine and the call with a new
C XD, YD, and ZD array must be made with MD=1.  The call with MD=2
C must be preceded by another call with the same XD, YD, and ZD
C arrays.  Between the call with MD=2 and its preceding call, the
C WK array must not be disturbed.
C
...
C
C This subroutine calls the RGPD3P, RGLCTN, and RGPLNL subroutines.
C
C
...
C Error check
   ...
   20 CONTINUE
      ...
      IER = 0
C Calculation
C Estimates partial derivatives at all input-grid data points
C (for MD=1).
      IF (MD.NE.2) THEN
          CALL RGPD3P(NXD,NYD,XD,YD,ZD, WK)
C         CALL RGPD3P(NXD,NYD,XD,YD,ZD, PDD)
      END IF
C DO-loop with respect to the output point
C Processes NIPIMX output points, at most, at a time.
      DO 30 IIP = 1,NIP,NIPIMX
          NIPI = MIN(NIP- (IIP-1),NIPIMX)
C Locates the output points.
          CALL RGLCTN(NXD,NYD,XD,YD,NIPI,XI(IIP),YI(IIP), INXI,INYI)
C         CALL RGLCTN(NXD,NYD,XD,YD,NIP,XI,YI, INXI,INYI)
C Calculates the z values at the output points.
          CALL RGPLNL(NXD,NYD,XD,YD,ZD,WK,NIPI,XI(IIP),YI(IIP),INXI,
     +                INYI, ZI(IIP))
C         CALL RGPLNL(NXD,NYD,XD,YD,ZD,PDD,NIP,XI,YI,INXI,INYI, ZI)
   30 CONTINUE
      RETURN
C Error exit
   40 CONTINUE
...
C
C Rectangular-grid surface fitting
C (a master subroutine of the RGBI3P/RGSF3P subroutine package)
C
C Hiroshi Akima
C U.S. Department of Commerce, NTIA/ITS
C Version of 1995/08
C
C This subroutine performs surface fitting by interpolating
C values of a bivariate function, z(x,y), on a rectangular grid
C in the x-y plane.  It is based on the revised Akima method.
C
C In this subroutine, the interpolating function is a piecewise
C function composed of a set of bicubic (bivariate third-degree)
C polynomials, each applicable to a rectangle of the input grid
...
C
C This subroutine has the accuracy of a bicubic polynomial, i.e.,
C it fits the surface accurately when all data points lie on a
C surface of a bicubic polynomial.
C
C The grid lines of the input and output data can be unevenly
C spaced.
C
C The input arguments are
C   MD  = mode of computation
...
C         the interpolated z values at the output grid
...
C       = 4 for identical YD values or
C               YD values out of sequence
C       = 5 for NXI = 0 or less
C       = 6 for NYI = 0 or less.
C
C The other argument is
C   WK  = three-dimensional array of dimension 3*NXD*NYD used
C         internally as a work area.
...
C XD, YD, or ZD array must be made with MD=1.  The call with MD=2
C must be preceded by another call with the same XD, YD, and ZD
C arrays.  Between the call with MD=2 and its preceding call, the
C WK array must not be disturbed.
C
...
C         CALL RGPD3P(NXD,NYD,XD,YD,ZD, PDD)
      END IF
C Outermost DO-loop with respect to the y coordinate of the output
C grid points
      DO 50 IYI = 1,NYI
          DO 30 IXI = 1,NIPIMX
              YII(IXI) = YI(IYI)
   30     CONTINUE
C Second DO-loop with respect to the x coordinate of the output
C grid points
C Processes NIPIMX output-grid points, at most, at a time.
          DO 40 IXI = 1,NXI,NIPIMX
              NIPI = MIN(NXI- (IXI-1),NIPIMX)
C Locates the output-grid points.
              CALL RGLCTN(NXD,NYD,XD,YD,NIPI,XI(IXI),YII, INXI,INYI)
C             CALL RGLCTN(NXD,NYD,XD,YD,NIP,XI,YI, INXI,INYI)
C Calculates the z values at the output-grid points.
...
C     WRITE (*,FMT=9060) NXD,NYD,NXI,NYI
      ...
      DOUBLE PRECISION PURE FUNCTION Z2F(XX1,XX2,ZZ0,ZZ1) 
      DOUBLE PRECISION, INTENT(IN) :: XX1,XX2
      DOUBLE PRECISION, INTENT(IN) :: ZZ0,ZZ1
      Z2F=(ZZ1-ZZ0)*XX2/XX1 + ZZ0
      RETURN
      END

      DOUBLE PRECISION PURE FUNCTION Z3F(XX1,XX2,XX3,ZZ0,ZZ1,ZZ2)
      DOUBLE PRECISION, INTENT(IN) :: XX1,XX2,XX3
      DOUBLE PRECISION, INTENT(IN) :: ZZ0,ZZ1,ZZ2
...
C
C Partial derivatives of a bivariate function on a rectangular grid
C (a supporting subroutine of the RGBI3P/RGSF3P subroutine package)
C
C Hiroshi Akima
C U.S. Department of Commerce, NTIA/ITS
C Version of 1995/08
C
C This subroutine estimates three partial derivatives, zx, zy, and
C zxy, of a bivariate function, z(x,y), on a rectangular grid in
...
C   PDD = three-dimensional array of dimension 3*NXD*NYD,
C         where the estimated zx, zy, and zxy values at the
C         input-grid data points are to be stored.
C
C
C Specification statements
C     .. Scalar Arguments ..
...
C Data statements 
     ...
     +                 -2,-1,1,2,-1,1,2,3/
C     ..
C Statement Function definitions  -> converted above
C      Z2F(XX1,XX2,ZZ0,ZZ1) = (ZZ1-ZZ0)*XX2/XX1 + ZZ0
C      Z3F(XX1,XX2,XX3,ZZ0,ZZ1,ZZ2) = ((ZZ2-ZZ0)* (XX3-XX1)/XX2-
C     +                               (ZZ1-ZZ0)* (XX3-XX2)/XX1)*
C     +                               (XX3/ (XX2-XX1)) + ZZ0
C     ..
C initialize some variables to silence compiler warnings      
      Z00=0.0D0
      Z01=0.0D0
      Z02=0.0D0
      Z03=0.0D0
      ...
      Y3=0.0D0
C Calculation
C Initial setting of some local variables
      NX0 = MAX(4,NXD)
      NY0 = MAX(4,NYD)
C Double DO-loop with respect to the input grid points
      DO 60 IY0 = 1,NYD
          DO 50 IX0 = 1,NXD
              X0 = XD(IX0)
              Y0 = YD(IY0)
              Z00 = ZD(IX0,IY0)
C Part 1.  Estimation of ZXDI
C Initial setting
              SMPEF = 0.0
              SMWTF = 0.0
              SMPEI = 0.0
              SMWTI = 0.0
C DO-loop with respect to the primary estimate
              DO 10 IPEX = 1,4
C Selects necessary grid points in the x direction.
                  IX1 = IX0 + IDLT(1,IPEX)
                  IX2 = IX0 + IDLT(2,IPEX)
                  IX3 = IX0 + IDLT(3,IPEX)
     ...
     +                (IX3.GT.NX0)) GO TO 10
C Selects and/or supplements the x and z values.
                  X1 = XD(IX1) - X0
                  Z10 = ZD(IX1,IY0)
                  IF (NXD.GE.4) THEN
                      X2 = XD(IX2) - X0
                  ...
                  DZX20 = (Z20-Z00)/X2
                  DZX30 = (Z30-Z00)/X3
C Calculates the primary estimate of partial derivative zx as
C the coefficient of the bicubic polynomial.
                  CX1 = X2*X3/ ((X1-X2)* (X1-X3))
                  CX2 = X3*X1/ ((X2-X3)* (X2-X1))
                  CX3 = X1*X2/ ((X3-X1)* (X3-X2))
                  PEZX = CX1*DZX10 + CX2*DZX20 + CX3*DZX30
C Calculates the volatility factor and distance factor in the x
C direction for the primary estimate of zx.
                  SX = X1 + X2 + X3
                  SZ = Z00 + Z10 + Z20 + Z30
...
C not the volatility factor is essentially zero.
                  EPSLN = (Z00**2+Z10**2+Z20**2+Z30**2)*1.0E-12
C Accumulates the weighted primary estimates of zx and their
C weights.
                  IF (VOLF.GT.EPSLN) THEN
C - For a finite weight.
                      WT = 1.0/ (VOLF*DISF)
                      SMPEF = SMPEF + WT*PEZX
                      SMWTF = SMWTF + WT
                  ELSE
C - For an infinite weight.
                      SMPEI = SMPEI + PEZX
                      SMWTI = SMWTI + 1.0
                  END IF
C Saves the necessary values for estimating zxy
                  XA(1,IPEX) = X1
                  XA(2,IPEX) = X2
                  XA(3,IPEX) = X3
                  ZI0A(1,IPEX) = Z10
                  ...
                  B10A(IPEX) = B10
   10         CONTINUE
C Calculates the final estimate of zx.
...
C DO-loop with respect to the primary estimate
              DO 20 IPEY = 1,4
C Selects necessary grid points in the y direction.
                  IY1 = IY0 + IDLT(1,IPEY)
                  IY2 = IY0 + IDLT(2,IPEY)
                  IY3 = IY0 + IDLT(3,IPEY)
     ...
     +                (IY3.GT.NY0)) GO TO 20
C Selects and/or supplements the y and z values.
                  Y1 = YD(IY1) - Y0
                  Z01 = ZD(IX0,IY1)
                  IF (NYD.GE.4) THEN
                      Y2 = YD(IY2) - Y0
                  ...
                  DZY02 = (Z02-Z00)/Y2
                  DZY03 = (Z03-Z00)/Y3
C Calculates the primary estimate of partial derivative zy as
C the coefficient of the bicubic polynomial.
                  CY1 = Y2*Y3/ ((Y1-Y2)* (Y1-Y3))
                  CY2 = Y3*Y1/ ((Y2-Y3)* (Y2-Y1))
                  CY3 = Y1*Y2/ ((Y3-Y1)* (Y3-Y2))
                  PEZY = CY1*DZY01 + CY2*DZY02 + CY3*DZY03
C Calculates the volatility factor and distance factor in the y
C direction for the primary estimate of zy.
                  SY = Y1 + Y2 + Y3
                  SZ = Z00 + Z01 + Z02 + Z03
...
C Accumulates the weighted primary estimates of zy and their
C weights.
                  IF (VOLF.GT.EPSLN) THEN
C - For a finite weight.
                      WT = 1.0/ (VOLF*DISF)
...
C Saves the necessary values for estimating zxy
                  ...
                  B01A(IPEY) = B01
   20         CONTINUE
C Calculates the final estimate of zy.
              IF (SMWTI.LT.0.5) THEN
C - When no infinite weights exist.
                  ZYDI = SMPEF/SMWTF
              ELSE
C - When infinite weights exist.
              ...
              END IF
C End of Part 2.
C Part 3.  Estimation of ZXYDI
C Initial setting
              SMPEF = 0.0
              SMWTF = 0.0
              SMPEI = 0.0
              SMWTI = 0.0
C Outer DO-loops with respect to the primary estimates in the x
C direction
              DO 40 IPEX = 1,4
                  IX1 = IX0 + IDLT(1,IPEX)
                  IX2 = IX0 + IDLT(2,IPEX)
     ...
     +                (IX1.GT.NX0) .OR. (IX2.GT.NX0) .OR.
     +                (IX3.GT.NX0)) GO TO 40
C Retrieves the necessary values for estimating zxy in the x
C direction.
                  X1 = XA(1,IPEX)
                  X2 = XA(2,IPEX)
                  X3 = XA(3,IPEX)
                  ...
                  B00X = B00XA(IPEX)
                  B10 = B10A(IPEX)
C Inner DO-loops with respect to the primary estimates in the y
C direction
                  DO 30 IPEY = 1,4
                      IY1 = IY0 + IDLT(1,IPEY)
                      IY2 = IY0 + IDLT(2,IPEY)
...
C Retrieves the necessary values for estimating zxy in the y
C direction.
                      Y1 = YA(1,IPEY)
                      Y2 = YA(2,IPEY)
                      Y3 = YA(3,IPEY)
                      ...
                      B00Y = B00YA(IPEY)
                      B01 = B01A(IPEY)
C Selects and/or supplements the z values.
                      IF (NYD.GE.4) THEN
                          Z11 = ZD(IX1,IY1)
                          Z12 = ZD(IX1,IY2)
                          Z13 = ZD(IX1,IY3)
                      ...
                      END IF
C Calculates the primary estimate of partial derivative zxy as
C the coefficient of the bicubic polynomial.
...
C Calculates the volatility factor and distance factor in the x
C and y directions for the primary estimate of zxy.
                      B00 = (B00X+B00Y)/2.0
                      SXY = SX*SY
                      SXXY = SXX*SY
                      SXYY = SX*SYY
...
C Calculates EPSLN.
     ...
     +                        1.0E-12
C Accumulates the weighted primary estimates of zxy and their
C weights.
                      IF (VOLF.GT.EPSLN) THEN
C - For a finite weight.
                          WT = 1.0/ (VOLF*DISF)
...
C - For an infinite weight.
   ...
   30             CONTINUE
   40         CONTINUE
C Calculates the final estimate of zxy.
              IF (SMWTI.LT.0.5) THEN
C - When no infinite weights exist.
                  ZXYDI = SMPEF/SMWTF
              ELSE
...
C
C Location of the desired points in a rectangular grid
C (a supporting subroutine of the RGBI3P/RGSF3P subroutine package)
C
C Hiroshi Akima
C U.S. Department of Commerce, NTIA/ITS
C Version of 1995/08
C
C This subroutine locates the desired points in a rectangular grid
C in the x-y plane.
C
C The grid lines can be unevenly spaced.
C
...
C          of the input-grid data points (must be in a
C          monotonic increasing order),
C   NIP  = number of the output points to be located (must be
C          1 or greater),
C   XI   = array of dimension NIP containing the x coordinates
C          of the output points to be located,
C   YI   = array of dimension NIP containing the y coordinates
C          of the output points to be located.
C
C The output arguments are
C   INXI = integer array of dimension NIP where the interval
C          numbers of the XI array elements are to be stored,
C   INYI = integer array of dimension NIP where the interval
C          numbers of the YI array elements are to be stored.
C The interval numbers are between 0 and NXD and between 0 and NYD,
C respectively.
C
C
C Specification statements
...
C initialize some variables to silence compiler warnings      
      IXD=0
      IYD=0
C DO-loop with respect to IIP, which is the point number of the
C output point
      DO 30 IIP = 1,NIP
          XII = XI(IIP)
          YII = YI(IIP)
C Checks if the x coordinate of the IIPth output point, XII, is
C in a new interval.  (NINTX is the new-interval flag.)
          IF (IIP.EQ.1) THEN
              NINTX = 1
          ELSE
              NINTX = 0
     ...
     +                (XII.GT.XD(IXD+1))) NINTX = 1
              ...
              END IF
          END IF
C Locates the output point by binary search if XII is in a new
C interval.  Determines IXD for which XII lies between XD(IXD)
C and XD(IXD+1).
          IF (NINTX.EQ.1) THEN
              IF (XII.LE.XD(1)) THEN
                  IXD = 0
   ...
   10             IF (XII.GE.XD(IMD)) THEN
          ...
          END IF
          INXI(IIP) = IXD
C Checks if the y coordinate of the IIPth output point, YII, is
C in a new interval.  (NINTY is the new-interval flag.)
          IF (IIP.EQ.1) THEN
              NINTY = 1
          ELSE
              NINTY = 0
     ...
     +                (YII.GT.YD(IYD+1))) NINTY = 1
              ...
              END IF
          END IF
C Locates the output point by binary search if YII is in a new
C interval.  Determines IYD for which YII lies between YD(IYD)
C and YD(IYD+1).
          IF (NINTY.EQ.1) THEN
              IF (YII.LE.YD(1)) THEN
                  IYD = 0
...
C
C Polynomials for rectangular-grid bivariate interpolation and
C surface fitting
C (a supporting subroutine of the RGBI3P/RGSF3P subroutine package)
C
C Hiroshi Akima
...
C          points,
C   PDD  = three-dimensional array of dimension 3*NXD*NYD
C          containing the estimated zx, zy, and zxy values
C          at the input-grid data points,
C   NIP  = number of the output points at which interpolation
...
C   YI   = array of dimension NIP containing the y coordinates
C          of the output points,
C   INXI = integer array of dimension NIP containing the
C          interval numbers of the input grid intervals in the
C          x direction where the x coordinates of the output
C          points lie,
C   INYI = integer array of dimension NIP containing the
C          interval numbers of the input grid intervals in the
C          y direction where the y coordinates of the output
C          points lie.
C
C The output argument is
C   ZI   = array of dimension NIP, where the interpolated z
...
C Outermost DO-loop with respect to the output point
      DO 10 IIP = 1,NIP
          ...
          IXDI = INXI(IIP)
          IYDI = INYI(IIP)
C Retrieves the z and partial derivative values at the origin of
C the coordinate for the rectangle.
          ...
          END IF
C Case 1.  When the rectangle is inside the data area in both the
C x and y directions.
          IF ((IXDI.GT.0.AND.IXDI.LT.NXD) .AND.
     +        (IYDI.GT.0.AND.IYDI.LT.NYD)) THEN
C Retrieves the z and partial derivative values at the other three
C vertexes of the rectangle.
              IF (IXDI.NE.IXDIPV .OR. IYDI.NE.IYDIPV) THEN
                  IXD1 = IXD0 + 1
                  DX = XD(IXD1) - X0
                  DXSQ = DX*DX
...
C Calculates the polynomial coefficients.
                  Z0DX = (Z10-Z00)/DX
                  Z1DX = (Z11-Z01)/DX
                  Z0DY = (Z01-Z00)/DY
                  Z1DY = (Z11-Z10)/DY
...
C End of Case 1
C Case 2.  When the rectangle is inside the data area in the x
C direction but outside in the y direction.
          ELSE IF ((IXDI.GT.0.AND.IXDI.LT.NXD) .AND.
     +             (IYDI.LE.0.OR.IYDI.GE.NYD)) THEN
C Retrieves the z and partial derivative values at the other
C vertex of the semi-infinite rectangle.
              IF (IXDI.NE.IXDIPV .OR. IYDI.NE.IYDIPV) THEN
                  IXD1 = IXD0 + 1
                  DX = XD(IXD1) - X0
                  DXSQ = DX*DX
...
C Evaluates the polynomial.
              ...
              ZII = Q0 + U* (Q1+U* (Q2+U*Q3))
C End of Case 2
C Case 3.  When the rectangle is outside the data area in the x
C direction but inside in the y direction.
...
C Evaluates the polynomial.
              ...
              ZII = Q0 + U*Q1
C End of Case 3
C Case 4.  When the rectangle is outside the data area in both the
C x and y direction.
          ELSE IF ((IXDI.LE.0.OR.IXDI.GE.NXD) .AND.
     +             (IYDI.LE.0.OR.IYDI.GE.NYD)) THEN
C Calculates the polynomial coefficients.
